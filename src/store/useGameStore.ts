...
import { getFestivalByDay, getUpcomingFestival, FESTIVAL_SNACKS, FESTIVAL_STORIES } from '@/data/festivals'
import { calcFestivalSpoilage } from '@/utils/festivalBonus'
...
function pickRandomStories(count: number, festival: Festival | null): Story[] {
  const pool = [...STORIES]
  const result: Story[] = []

  if (festival) {
    const festivalStories = FESTIVAL_STORIES.filter(
      (s) => s.festivalOnly === festival.id
    )
    for (const fs of festivalStories) {
      result.push(fs)
    }
  }
...
function buildSnackList(festival: Festival | null): Snack[] {
  const base = initSnacks()
  if (!festival) return base

  const festivalSnacks = FESTIVAL_SNACKS.filter(
    (s) => s.festivalOnly === festival.id
  ).map((s) => ({
    ...s,
  ...
  return [...base, ...festivalSnacks]
}

function resolveFestival(day: number): {
  currentFestival: Festival | null
  upcomingFestival: Festival | null
} {
  return {
    currentFestival: getFestivalByDay(day),
    upcomingFestival: getUpcomingFestival(day),
  }
}

function createInitialState(): GameState {
  const { currentFestival, upcomingFestival } = resolveFestival(1)
  return {
    day: 1,
    phase: 'day',
    gold: 200,
    reputation: 30,
    weather: '晴',
    snacks: buildSnackList(currentFestival),
    seats: initSeats(),
    ...
    (set, get) => ({
      ...
      buySnack: (snackId: string, qty: number) => {
        ...
        set((s) => ({
          ...
          festivalPrepared: s.festivalPrepared || !!(
            snack.festivalOnly &&
            s.currentFestival &&
            snack.festivalOnly === s.currentFestival.id
          ),
        }))
      ...
      switchToNight: () => {
        ...
        if (state.currentFestival) {
          customerCount += state.currentFestival.customerBonus
        }

        const customers = generateRandomCustomers(customerCount)

        if (state.currentFestival) {
          const fest = state.currentFestival
          for (const c of customers) {
            const extraTags = fest.preferredTags.filter(
              (t) => !c.preferenceTags.includes(t)
            )
            if (extraTags.length > 0) {
              const addTag = extraTags[Math.floor(Math.random() * extraTags.length)]
              c.preferenceTags = [...c.preferenceTags, addTag]
            }
          }
        }
        ...
        const availableStories = pickRandomStories(3, state.currentFestival)
      ...
      selectStory: (storyId: string, branchId: string) => {
        ...
        set({
          ...
          storyProgress: 0,
          festivalPrepared: get().festivalPrepared || !!(
            state.currentFestival &&
            story.festivalOnly === state.currentFestival.id
          ),
        })
      },
      ...
      tickPerformance: () => {
        ...
        const customers = state.customers.map((c) => {
          ...
          if (state.currentFestival) {
            const festTagMatch = state.currentFestival.preferredTags.some(
              (t) => c.preferenceTags.includes(t)
            )
            if (festTagMatch) delta += 1
          }
      ...
      doSettlement: () => {
        ...
        let repReason = result.reputationDelta >= 0 ? '说书好评' : '差评影响'
        if (state.currentFestival && state.festivalPrepared && result.reputationDelta > 0) {
          repReason = `${state.currentFestival.name}节令热潮`
        } else if (state.festivalMissed && result.reputationDelta < 0) {
          repReason = `错失${state.currentFestival?.name ?? '节令'}口碑`
        }
        ...
        if (result.festivalBonus > 0)
          get().addLedgerRecord('收入', '节令热潮', result.festivalBonus, `${state.currentFestival?.name ?? ''}加成`)
      },

      nextDay: () => {
        const state = get()
        const prevFestival = state.currentFestival
        const newDay = state.day + 1
        const { currentFestival, upcomingFestival } = resolveFestival(newDay)

        let snacks = state.snacks.map((s) => ({ ...s }))

        const { spoiledSnacks, goldLost } = calcFestivalSpoilage(
          snacks,
          prevFestival,
          currentFestival
        )

        if (spoiledSnacks.length > 0) {
          for (const spoiled of spoiledSnacks) {
            const idx = snacks.findIndex((s) => s.id === spoiled.id)
            if (idx >= 0) snacks[idx] = { ...snacks[idx], stock: spoiled.stock }
          }
        }

        snacks = snacks.filter(
          (s) => !s.perishable || !s.festivalOnly || s.stock > 0 || currentFestival?.id === s.festivalOnly
        )

        if (currentFestival) {
          const existingIds = new Set(snacks.map((s) => s.id))
          const newFestivalSnacks = FESTIVAL_SNACKS.filter(
            (s) => s.festivalOnly === currentFestival.id && !existingIds.has(s.id)
          ).map((s) => ({ ...s, stock: Math.floor(s.maxStock * 0.3) }))
          snacks = [...snacks, ...newFestivalSnacks]
        }
        ...
        set((s) => ({
          ...
          currentFestival,
          upcomingFestival,
          festivalMissed: !!prevFestival && !s.festivalPrepared,
          festivalPrepared: false,
          snacks,
        ...
        if (goldLost > 0) {
          get().addLedgerRecord('支出', '节令积压', goldLost, '限定茶点过期损耗')
        }
      },

      resetGame: () => {
        set({ ...createInitialState(), weather: randomWeather() })
      },
